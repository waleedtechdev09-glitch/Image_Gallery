...
//     folderIdFromUrl,
//   );
//   const [breadcrumb, setBreadcrumb] = useState<string>(""); //  system/files/subfiles
      ...
      <main className="flex-1 p-6 overflow-y-auto">
        ...
        {breadcrumb && (
          <nav className="mb-4 flex items-center text-sm text-gray-600 space-x-2">
            ...
            {breadcrumb.split("/").map((name, idx, arr) => (
              <span key={idx} className="flex items-center">
                <span
                  className={`text-orange-600 cursor-pointer font-medium hover:underline`}
                  onClick={() => {
                    // Clicking a breadcrumb part resolves that folder
                    ...
                    axios
                      ...
                      .then((res) => setSelectedFolder(res.data.folderId))
                      .catch(console.error);
                  }}
                >
                  {name}
